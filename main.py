...
for i, name in enumerate(contatos_df['nome']):
    ...
    search_input = browser.find_element(By.XPATH, '//*[@id="side"]/div[1]/div/div[2]/div[2]/div/div[1]/p')
    search_input.send_keys(str(contact_phone))
    time.sleep(5)

    # click on the first match given should have only one
    browser.find_element(By.XPATH, '//*[@id="pane-side"]/div[1]/div/div/div[1]/div/div/div/div[1]').click()
    time.sleep(5)
    
    # write the messages
    print('writing messages')
    text_field = browser.find_element(By.XPATH, '//*[@id="main"]/footer/div[1]/div/span[2]/div/div[2]/div[1]/div/div[1]/p')
    text_field.send_keys(message_hello.format(contact_name))
    ...
    time.sleep(3)
    browser.find_element(By.CSS_SELECTOR, 'input[type="file"]').send_keys(os.path.abspath(f"data\\{contact_file}"))
    browser.find_element(By.CSS_SELECTOR, 'input[type="file"]').send_keys(Keys.ENTER)
    browser.find_element(By.CSS_SELECTOR, 'data-icon="send"').click()
